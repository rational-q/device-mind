...
-- 11. 场景联动定义表
CREATE TABLE IF NOT EXISTS dm_scene (
                                        ID BIGINT NOT NULL COMMENT '主键，应用层雪花ID',
                                        ...
                                        ACTIONS JSON NOT NULL COMMENT '执行动作JSON',
                                        ENABLED TINYINT DEFAULT 1 COMMENT '是否启用',
                                        CREATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                        CREATED_BY BIGINT COMMENT '创建人ID',
                                        UPDATED_DATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                        UPDATED_BY BIGINT COMMENT '更新人ID',
                                        PRIMARY KEY (ID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='场景联动定义表';

-- 12. 场景执行日志表
CREATE TABLE IF NOT EXISTS dm_scene_log (
                                            ID BIGINT NOT NULL COMMENT '主键，应用层雪花ID',
                                            SCENE_ID BIGINT NOT NULL COMMENT '场景ID',
                                            SCENE_NAME VARCHAR(100) COMMENT '场景名称',
                                            DEVICE_ID VARCHAR(64) NOT NULL COMMENT '触发设备ID',
                                            TRIGGERED_AT TIMESTAMP NOT NULL COMMENT '触发时间',
                                            ACTIONS_RESULT TEXT COMMENT '动作执行结果JSON',
...
-- 产品
INSERT IGNORE INTO dm_product (ID, PRODUCT_KEY, NAME, DESCRIPTION, CREATED_BY) VALUES
...
-- 物模型属性
INSERT IGNORE INTO dm_thing_attribute (ID, PRODUCT_ID, IDENTIFIER, NAME, DATA_TYPE, UNIT, ACCESS_MODE, DESCRIPTION, CREATED_BY) VALUES
...
-- 告警规则
INSERT IGNORE INTO dm_alert_rule (ID, RULE_NAME, DEVICE_TYPE, ATTR_NAME, OPERATOR, THRESHOLD, DURATION_SECONDS, LEVEL, CREATED_BY) VALUES
...
-- 场景联动（预置示例）
INSERT IGNORE INTO dm_scene (ID, NAME, DESCRIPTION, PRODUCT_ID, CONDITIONS, ACTIONS, ENABLED, CREATED_BY) VALUES
    (301, '高温自动排风', '3号车间温度超过40℃时自动打开排风扇，并发送短信通知',
     1,
     '[{"attr":"temperature","operator":">","value":40,"duration":30}]',
     '[{"type":"COMMAND","targetDeviceId":"FAN-01","command":"turnOn","params":{}},{"type":"DELAY","seconds":10},{"type":"SMS","phoneNumbers":["13800138000"],"content":"3号车间温度异常，已自动打开排风扇"}]',
     1, 0);

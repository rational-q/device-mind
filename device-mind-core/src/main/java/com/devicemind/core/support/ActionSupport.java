...
public class ActionSupport {
    ...
    public List<Map<String, Object>> execute(String actionsJson) {
        ...
        try {
            List<Map<String, Object>> actions = JsonUtil.fromJson(actionsJson,
    ...
    private void executeCommand(Map<String, Object> action, Map<String, Object> result) {
        ...
        try {
            String paramsJson = !params.isEmpty()
                    ? JsonUtil.toJson(params) : "{}";
            deviceCommandProducer.sendCommandAsync(targetDeviceId, command, paramsJson, idempotencyKey);
            result.put("success", true);
            result.put("targetDeviceId", targetDeviceId);
            result.put("command", command);
            result.put("message", "指令已发送");
        } catch (Exception e) {
            log.error("指令发送失败: deviceId={}, command={}", targetDeviceId, command, e);
            result.put("success", false);
            result.put("targetDeviceId", targetDeviceId);
            result.put("command", command);
